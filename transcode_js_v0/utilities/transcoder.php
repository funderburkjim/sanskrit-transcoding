...
//  first character of $key
 ...
 foreach($fsmentries as $i => $fsmentry) {
  $in = $fsmentry['in'];
  //$c = $in[0]; // changed to next 02-04-2024. So JSON works in createJson2
  $c = mb_substr($in, 0, 1);
  $state=$states[$c];
  if ($state) {
    $state[]=$i;
 ...
 $fsm['states']=$states;
 if ($dbg) {
  $fsmkeys=array_keys($fsm);
  print("fsm keys=" . join(" ",$fsmkeys));
  var_dump($fsm);
 }
...
function transcoder_processString_main($line,$fsm) {
 ...
 // $line as array of unicode utf-8 characters 02-04-2024
 $linechars = preg_split('//u', $line, -1, PREG_SPLIT_NO_EMPTY);
 $m = count($linechars);
 //$m=strlen($line);
 while ($n < $m) {
  // $c = $line[$n];
  ...
  foreach($isubs as $isub) {
   ...
   $match = transcoder_processString_match($linechars,$n,$m,$fsmentry);
   $nmatch=strlen($match);
...
function transcoder_processString_match($linechars,$n,$m,$fsmentry) {
  $match=""; // value returned
  $edge = $fsmentry['in']; // unicode string
  //$nedge=strlen($edge);
  $edgechars = preg_split('//u', $edge, -1, PREG_SPLIT_NO_EMPTY);
  $nedge = count($edgechars);
  $j=$n;
  $k=0;
  $b=TRUE;
  while ( ($j < $m) && ($k < $nedge) && $b) {
   if($linechars[$j] == $edgechars[$k]) {
    $j++;
    $k++;
   }else {
    $b=FALSE;
