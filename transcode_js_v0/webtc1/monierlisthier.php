...
$parms =array( $keyin, $direction,$keyboard,$filter ,$filterin);
...
exit(0);
/*---------------------------
  test version Nov 7, 2013
----------------------------
*/
function listhier_test($parms) {
list($keyin, $direction,$keyboard,$filter ,$filterin)=$parms;
...
while($i < count($listmatches)) {
 ...
 if (preg_match('/^H([2])/',$h2,$matches)) {
  $spc="$spcchar";
 }else if(preg_match('/^H([3])/',$h2,$matches)) {
  $spc="$spcchar$spcchar";
 }else if(preg_match('/^H([4])/',$h2,$matches)) {
  $spc="$spcchar$spcchar$spcchar";
 }else {
  $spc="";
 }
 if (is_float($lnum2)) {
  // Nov 15, 2013.  Otherwise, found dvizWatA as 98740.100000001
  // Is this a sqlite artifact ?
  $lnum2 = round($lnum2,2);
 }
 $spc = "<span style='font-size:x-small;}'>$h2 $lnum2 </span>"; // test
 ...
 if (preg_match('|<phw>.*?<pL>|',$data2)) {
  $out1 = "<span style='background-color:LightGreen;'>$out1</span>";
 }else if (preg_match('|<phw>.*?<dL>|',$data2)) {
  $out1 = "<span style='background-color:Beige;'>$out1</span>";
 }
 $table .= $out1;
...
function list_filter_test($lnum0,$recarr) {
global $test;
...
foreach($recarr as $rec) {
  list($key1,$lnum1,$data1) = $rec;
/*
  if (preg_match('/^<H.A>/',$data1)) {continue;} 
...
global $test;
if ($test) {
 return list_filter_test($lnum0,$recarr);
}
// This variant matches on key+hom
$matches=array();
$recarr0 = dal_monier2($lnum0,$lnum0); 
