<?php
//sanskrit/monier/disp2/test_monierlisthier.php
//ejf Apr 6, 2012
...
/*  Program Overview
  Inputs:  key
           direction (UP, DOWN,CENTER (default))
  Program operation for CENTER:
    list($key1,$lnum1,$data1) from $key, via match_key
    $listmatches = list_center($key1,$lnum1,$data1,12,12)
    Format listmatches, and output.
  list_center($key1,$lnum1,$data1,$nprev,$nnext) :
   $matches1 = list_prev($key1,$lnum1,$nprev);
   $matches2 = list_next($key1,$lnum1,$nnext);

   list_prev($key0,$lnum0,$nprev):
    via dal_monier4a($lnum0,5*$nprev), get $recarr as
      array of mw records in descending order:
    with $max = 5*$nprev,
     "select * from monier where (lnum < '$lnum0') order by lnum DESC LIMIT $max"
     $matches= list_filter($lnum0,$recarr)
     return $matches, resorted into ascending order

   list_next($key0,$lnum0,$n0):
    via dal_monier4b($lnum0,5*$n0), get $recarr as
      array of mw records in ascending order:
    with $max = 5*$n0, 
     "select * from monier where ('$lnum0' < lnum) order by lnum LIMIT $max"
     $matches= list_filter($lnum0,$recarr)
     return $matches

   list_filter($lnum0,$recarr):
...
while($i < count($listmatches)) {
 ...
 $out1 = "$spc<a  onclick='getWordAlt_keyboard(\"<SA>$key2</SA>\");'><span style='$c'><SA>$key2</SA></span>$hom2</a><br/>\n";
 // test
  $out1 = "list($i): $key2\n";
...
echo "CALLING list_filter in list_prev...\n";
$matches = list_filter($lnum0,$recarr);
...
// 1. get records to be displayed
echo "CALLING list_filter in list_next...\n";
$matches = list_filter($lnum0,$recarr);
// 2. get the last $nprev records for return
$nmatches = count($matches);
...
//list($key0,$lnum0a,$data0)=$recarr[0];  // shld this be $recarr0 ?
list($key0,$lnum0a,$data0)=$recarr0[0];  // Yes!
...
foreach($recarr as $rec) {
  ...
  list($key1,$lnum1,$data1) = $rec;
  echo "filter: $key1 ";
  if (!preg_match('/^<H[1-4][BC]?>/',$data1)) {echo "(DROP1)\n";continue;}
  $keyhom1 = get_keyhom($key1,$data1);
  if ($keyhom1 == $keyhom){echo "(DROP2, '$keyhom1')\n";continue;}
  if ($keyhom1 == $keyhom0) {echo "(DROP3, '$keyhom1')\n";continue;}
  // found a new one
  echo "(KEPT)\n";
  $matches[]=$rec;
  $keyhom = $keyhom1; 
