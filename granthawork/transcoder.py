...
def transcoder_fsm(sfrom,to) :
 ...
 if sfrom.startswith('slp1') and to.startswith('deva'):
  ...
  regexCode = 'hkt_tamil'
 elif sfrom.startswith('slp1') and to.startswith('grantha'):
  regexCode = 'slp1_grantha'
 ...
 for e in entries:
  ...
  # four hex digits.
  #print('dbg: inval=%s, outval=%s' %(inval,outval)) ## ejfxxx
...
def transcoder_processString_match(line,n,m,fsmentry) :
  ...
  if (fsmentry['regex'] in ['slp1_deva','slp1_grantha']) :
